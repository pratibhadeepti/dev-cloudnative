...
def get_server_data(inventory, server):
    if (server == 'admin'):
        suffix = os.environ.get('ADMIN_INSTANCE_SUFFIX')
        a_num = int(os.environ.get('ADMIN_INSTANCE_VALUE')) + 1
    elif (server == 'sub'):
        suffix = os.environ.get('SUB_INSTANCE_SUFFIX')
        a_num = int(os.environ.get('SUB_INSTANCE_VALUE')) + 1
    else:
        print("Wrong value!")
    host = os.environ.get('K8S_CLUSTER_NAME') + '-' + suffix + '-'
    output = ""
    for i in range(1,a_num):
        hostvar = host + `i`
        result = to_json(inventory.get_host(hostvar))
        data = json.loads(result)
        if (data == None):
            ip =""
        else:
            ip = str(data['private_v4'])
        output += hostvar + " ansible_host=" + ip + '\n'
    return output
...
def parse_args():
    ...
    group.add_argument('--admin', action='store_true',
                       help='List details about admin servers')
    group.add_argument('--sub', action='store_true',
                       help='List details about customer servers')
...
def main():
    ...
    try:
        ...
        if args.list:
            ...
            output = get_server_data(inventory, 'admin')
        elif args.sub:
            output = get_server_data(inventory, 'sub')
        print(output)
